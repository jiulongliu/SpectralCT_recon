function X=ReconNLTVmultibins(y0,x0,mu0,lambda,GPU)
[nd,nv0,nbin]=size(y0);
...
para=struct('version',uint32(1),'GPU',uint32(GPU),'SO',single(SO),'OD',single(OD),'scale',single(scale),'nx',uint32(nx),'ny',uint32(ny),'nv',uint32(nv),...
    ...
    'cos_phi',[],'sin_phi',[],'cos_det',[],'sin_det',[],'nbin',uint32(nbin),'nd',uint32(nd));
...
varW=computeweight(single(reshape(x0,[nx ny])));
wps=struct('isC',uint32(0),'nx',uint32(nx),'ny',uint32(ny),'nt',uint32(nbin*10),'nb',uint32(nbin));
wps.W=varW.W;
wps.Y=varW.Y;
wps.SY=varW.SY;
wps.VecParametersNLTV=varW.VecParametersNLTV;

Wxs=@Wxnltvm3d;Wtxs=@Wtxnltvm3d;
...
'Min_iter',15,'threshold',1e-3,'N_iter',N_iter,'N',nx*ny*nbin,'nplot',nplot,'plotx',plotx,'var_plot',var_plot,...
'cg_iter',25,'cg_tol',1e-4,'JtJ',JtJ,'Wxs',Wxs,'Wtxs',Wtxs,'wps',wps,'Sxs',Sxs);
...
tic;[X n_iter d Ind Ind2]=bregman_tv([],y0,ip,var_CG);toc;  
X=reshape(X,[nx,ny,nbin]);
% save('Xrec0.mat','X');
% Xr=[Xr X];
% Xr=Xr(:,1:3*256);
% save('recX.mat','Xr');
